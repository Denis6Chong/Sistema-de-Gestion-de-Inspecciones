...
def menu_item(text: str, url: str) -> rx.Component:
    ...
    active = (rx.State.router.page.path == url.lower()) | (
        (rx.State.router.page.path == "/") & text == "Indice"
    )
    ...
    return rx.link(
        rx.hstack(
            rx.match(
                text,
                ("Indice", menu_item_icon("home")),
                ("Inspecciones", menu_item_icon("text-search")),
                ("Organismos", menu_item_icon("map-pin")),
                ("Entidades", menu_item_icon("map-pin")),
                ("Lineamientos", menu_item_icon("hash")),
                ("Establecimientos", menu_item_icon("map-pin")),
                ("Informes", menu_item_icon("table-2")),
                ("Requisitos", menu_item_icon("book_open")),
                ("Obligaciones", menu_item_icon("shield")),
                ("Normas", menu_item_icon("book")),
                ("Inspectores", menu_item_icon("user-round-search")),
...
def navbar() -> rx.Component:
    ...
    return rx.el.nav(
        rx.hstack(
            ...
            rx.image(
            src="/logo_color.svg", 
            width="5em",  # Ajusta el tamaño del logo
            height="auto",
            margin_top="10px"
        ),
            rx.spacer(),
